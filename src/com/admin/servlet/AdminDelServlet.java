...
public class AdminDelServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        AdminUserInstantiation adminisexist = adminUserQuery.queryadminid(sqladmin,delid);
        ...
        if(k==0){
            if(adminisexist.getId()!=0){//当前用户可以删除
                String sqlin = "delete from adminuser where id = ?";
                updata updata = new updata();
                updata.updateutil(sqlin, delid);
                pw.print(1);
            }
            else{
                pw.print(0);
